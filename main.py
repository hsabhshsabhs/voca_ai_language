...
# --- CONFIG ---
SECRET_KEY = "lingvo_recovery_real_2026_fixed_v2"
ALGORITHM = "HS256"
...
# МАКСИМАЛЬНО ОТКРЫТЫЙ CORS ДЛЯ ТЕСТОВ
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
@app.get("/health")
def health(): return {"status": "ok", "msg": "API is alive"}
...
def recovery_request(req: RecoveryRequest, db: Session = Depends(get_db)):
    user = db.query(User).filter(User.email == req.email).first()
    if not user: raise HTTPException(status_code=404)
    code = str(random.randint(100000, 999999))
    user.reset_code = code
    db.commit()
    # Эмуляция отправки (код в логах + SMTP)
    print(f"RECOVERY CODE FOR {user.email}: {code}")
    return {"status": "ok"}
...
async def chat_stream(req: ChatRequest, token: str):
    ...
    async def gen():
        ...
        history = [{"role": "system", "content": f"YOU ARE: {req.character}. SCENARIO: {req.situation}. Speak ONLY English."}]
        history.extend([{"role": m["role"], "content": m["content"]} for m in req.history if m.get("content")])
        if len(history) == 1: history.append({"role": "user", "content": "Start the conversation."})
        ...
        t_res = await deepseek_call([{"role":"user", "content":f"Translate to Russian: {full_en}"}])
        s_res = await deepseek_call([{"role":"user", "content":f"Context: {full_en}. Give 2 replies. JSON array."}])
        yield "||META||" + json.dumps({"translation": t_res, "suggestions": []}) # Упростил для теста
