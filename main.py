...
async def auth_telegram(req: dict, db: Session = Depends(get_db)):
    ...
    if not user:
        user = User(telegram_id=tg_id, username=user_data.get("username"), first_name=user_data.get("first_name"), credits=20.0)
...
async def chat_stream(req: dict, token: str, db: Session = Depends(get_db)):
    ...
    async def gen():
        ...
        t_task = asyncio.create_task(deepseek_call([{"role":"system", "content":"Translator. Russian ONLY."}, {"role":"user", "content": f"Translate: {full_en}"}]))
        s_task = asyncio.create_task(deepseek_call([{"role":"system", "content":"Return ONLY JSON array of 2 short English reply options. Format: [{\"en\":\"...\", \"ru\":\"...\"}]."}, {"role":"user", "content": f"Context: {full_en}"}]))
