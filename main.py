...
async def chat_stream(req: dict, token: str, db: Session = Depends(get_db)):
    ...
    async def gen():
        ...
        # Tiny delay to ensure content stream is finished before meta
        ...
        sug = []
        try: 
            match = re.search(r'\[\s*\{.*\}\s*\]', str(sug_raw), re.DOTALL)
            if match:
                parsed = json.loads(match.group(0))
                if isinstance(parsed, list): sug = parsed[:2]
        except: pass

        # Robust parsing for corrections
        corr_data = None
        if corr_raw and "NONE" not in str(corr_raw):
            try:
                match = re.search(r'\{\s*".*"\s*\}', str(corr_raw), re.DOTALL)
